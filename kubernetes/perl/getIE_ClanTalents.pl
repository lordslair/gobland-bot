...
foreach my $db (@db_list)
{
    ...
    if (%CREDENTIALS)
    {
        ...
        logEntry("[getIE_ClanTalents] DB: $db | Gob: $gob_rand");

        my $browser = new LWP::UserAgent;
        my $request = new HTTP::Request( GET => "http://ie.gobland.fr/IE_ClanTalents?id=$gob_rand&passwd=$CREDENTIALS{$gob_rand}" );
        ...
        if ($response->is_success)
        {
            foreach my $line (split(/\n/,$response->content))
            {
                ...
                if ( $line !~ /^#/ )
                {
                    my $sth  = $dbh->prepare( "REPLACE INTO Skills VALUES( '$line[0]-$line[1]-$line[2]', \
                                                                           '$line[0]', \
                                                                           '$line[1]', \
                                                                           '$line[2]', \
                                                                           '$line[3]', \
