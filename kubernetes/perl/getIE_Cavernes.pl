...
foreach my $db (@db_list)
{
    ...
    if (%CREDENTIALS)
    {
        ...
        foreach my $gob_id ( sort keys %CREDENTIALS )
        {
            ...
            logEntry("[getIE_Cavernes] DB: $db | Gob: $gob_id");

            my $browser = new LWP::UserAgent;
            my $request = new HTTP::Request( GET => "http://ie.gobland.fr/IE_Cavernes?id=$gob_id&passwd=$CREDENTIALS{$gob_id}" );
            ...
            if ($response->is_success)
            {
                foreach my $line (split(/\n/,$response->content))
                {
                    ...
                    if ( $line !~ /^#/ )
                    {
                        ...
                        if ( !$line[11]         ) { $line[11]    = '' } # Patch for Empty Reservation
                        if ( !$line[12]         ) { $line[12]    = '' } # Patch for Empty Matiere

                        # First query to INSERT the line if not exists, or IGNORE
                        my $sth  = $dbh->prepare( "INSERT IGNORE INTO Cavernes VALUES( '$line[0]',     \
                                                                                       '$line[1]',     \
                                                                                       '$line[2]',     \
                                                                                       '$line[3]',     \
                                                                                       '$line[4]',     \
                        ...
                        $sth       = $dbh->prepare( "UPDATE Cavernes SET Reservation = '$line[11]' \
                                                     WHERE  Id = '$line[0]' ");
        ...
        my $req_item_ids = $dbh->prepare( "SELECT Id FROM Cavernes WHERE Id > 0" );
        ...
        for my $item_id (keys %count)
        {
            if ( $count{$item_id} == 1 )
            {
                logEntry("[getIE_Cavernes]    CavernesCleaner:$item_id:$count{$item_id}");
                my $sth  = $dbh->prepare( "DELETE FROM Cavernes WHERE Id = '$item_id'" );
                   $sth->execute();
                   $sth->finish();
            }
        }
    ...
    {
        logEntry("[getIE_Cavernes] DB: $db | No credentials found");
    }
}
$dbh->disconnect();
